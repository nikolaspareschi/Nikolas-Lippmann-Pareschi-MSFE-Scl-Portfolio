# -*- coding: utf-8 -*-
"""
Created on Sat Mar 02 14:20:38 2019
...
#    n = 5
    ...
    Df['Signal'] = 0
    Df.loc[Df['Ret'] > Df['Ret'][:split].quantile(q=0.40), 'Signal'] = 1
    Df.loc[Df['Ret'] < Df['Ret'][:split].quantile(q=0.40), 'Signal'] = 0

    X = Df.drop(['Close', 'Signal', 'High', 'Low', 'Volume', 'Ret'], axis=1)
    # X=Df[['RSI','RSI_TLT','OC_SPY','OC_TLT']]
    # X=Df[['RSI','RSI_TLT','OO_SPY','OO_TLT']] 'OC_SPY',
    X = Df[['OC_TLT']]
    ...
    rcv = RandomizedSearchCV(pipeline, parameters, cv=3)
    ...
    Df['Strategy_Return2'].cumsum().plot(
            color='b', label='SPY Strategy returns')
    Df['Close'].pct_change(1)[split:].cumsum().plot(
            color='g', label='SPY returns')
    plt.legend(loc='best')
    ...
    yyy_spy = Output(Df['Ret1'].iloc[split:])
    print "\n The KPIs for our SPY strategy are:"
    print "are: \n\n", yyy_spy.generate_output()
